...
package object generatorTools {
  ...
  case class Spec(
                   ...
                   yamlPrefix: String,
                   rustOutFolder: Option[File],
                   rustJniOutFolder: Option[File])
  ...
  // We'll figure out which other things we need.
  case class RustIdentStyle(ty: IdentConverter,
  ...
  object IdentStyle {
    ...
    val rustDefault = RustIdentStyle(camelUpper, underLower, underLower, underLower, underCaps)
  ...
  def generate(idl: Seq[TypeDecl], spec: Spec): Option[String] = {
    try {
      ...
      if (spec.rustOutFolder.isDefined) {
        DEBUG(spec.rustOutFolder.get.toString)
        if (!spec.skipGeneration) {
...
abstract class Generator(spec: Spec)
{
  ...
  val idRust = IdentStyle.rustDefault // TODO(rustgen): allow specifying this
  ...
  def writeRustFileGeneric(folder: File)(name: String, origin: String) {
    createFile(folder, idRust.ty(name) + ".rs", (w: IndentWriter) => {
      w.wl("// AUTOGENERATED FILE - DO NOT MODIFY!")
      w.wl("// This file generated by Djinni from " + origin)
      w.wl
      w.wl("// TODO(rustgen): put stuff here")
    })
  }
